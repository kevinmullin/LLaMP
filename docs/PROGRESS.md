...
- Window is 275×116. Shade is `llamp_shade_height` (14). `EqWindowTests` checks both. `eqmain` and `eq_ex` origins are still absent from `docs/spec/skin-atlas.md`. The blit is a fallback fill, a curve from our peaking response, and slider hit regions from a phase-4 spacing formula. Those rectangles are not claimed as sprite slices. `layout_is_not_a_copied_eqmain_slot_table` fails if a slider x matches the usual copied slot list.
...
- Playlist formats in this slice are M3U8 only. Order and path strings round-trip. M3U, PLS, and XSPF are out. The URL menu item does not fetch. File info has no window.
