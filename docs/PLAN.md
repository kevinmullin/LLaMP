...
- Cold start to main window painted: ≤ 400 ms (phase 3). Cold start to first audible sample of an already-granted local file: ≤ 800 ms (phase 12).
...
Bundle size and launch-to-window are CI gates. Idle CPU and RSS are a phase-exit script on the `macos-26` runner. If that runner is too noisy to be fair, write how we measure. Do not raise the number.
...
- Phase 3 budgets in the Budgets section pass: launch-to-window, idle CPU, idle RSS, bundle size.
